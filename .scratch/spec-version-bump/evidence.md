...
- **H10**：`.gitignore` 進了變更集（`2bd80e6`，一行 `CLAUDE.local.md`），但它不在
  ...
  v1 核准。**不補進授權清單，理由是正確性不是成本**：事後把它加進清單會抹掉
  「這件事發生在授權之外」這個事實，讓紀錄比原本更不真實。這一列就是它的永久紀錄。
  **更正**：本報告先前在這裡寫「補進去要升版與再核准，交給核准者定價」，兩句都作廢。
  前一句把「檔案落在授權外」的補救說成升版——那正是本 scope 要修的行為，寫在這裡
  會教下一個讀者去做這份 SPEC 禁止的事。後一句說有一個待決的定價，但沒有：核准者
  只核准了 mutation 的降級，協調者在他們回答這兩項之前就封存，自己關掉了那條路。
  排序錯誤，責任在協調者。
...
- **H12**：SPEC v2「本版新授權什麼」第 3 項寫「補一條 `require`」，實作補了兩條
  ...
  淨增 4 條（90 → 94）：2 `forbid` ＋ 2 `require`，0 移除。
  **不回頭改 SPEC 的核准文字**，理由與 H10 同：改成「兩條」會讓紀錄看起來一致，
  但那是抹掉「實作比核准字面多做了一條」這個事實。這一列就是它的永久紀錄，
  不是待決事項。
