...
# 這裡用真正的 p10k external（釘 tag + sha256）走一遍。要網路：只有辨識得出的網路
# 失敗才 skip，模板錯、checksum 錯、權限錯都要留下 not ok。
...
_zwc_cm() { # subcommand... -> stdout 濾掉 config 提醒；回傳 chezmoi 自己的退出碼
    # 用 if 包住：run.sh 開了 set -e，失敗的 chezmoi 會直接結束 runner，退出碼交不回呼叫者。
    if chezmoi --source "$REPO" --config "$FIXTURES/os-linux.toml" --destination "$_zwc_dest" \
        --persistent-state "$_zwc_dest.state" --no-tty "$@" </dev/null > "$TMP/l5-zwc.out" 2>&1
    then _zrc=0; else _zrc=$?; fi
...
if [ "$_rc" -ne 0 ]; then
    case $_out in
        *"dial tcp"*|*"no such host"*|*"i/o timeout"*|*"network is unreachable"*|*"connection refused"*|*"connection reset"*|*"TLS handshake"*|*"Temporary failure"*)
            skip "p10k 的 .zwc 在 exact external 底下保留" "p10k external 抓不到（網路）：$(printf '%s' "$_out" | head -1)" ;;
        *) _fail "p10k external 套用成功（rc=$_rc）" "$_out" ;;
    esac
    ...
    _zwc_cm state set --bucket=entryState --key="$_zwc_p10k/internal/p10k.zsh.zwc" \
        --value='{"type":"remove"}' >/dev/null || _fail "種入 remove 記錄" "chezmoi state set 失敗"
    if _out=$(_zwc_cm status "$_zwc_p10k"); then _rc=0; else _rc=$?; fi
    if [ "$_rc" -eq 0 ] && [ -z "$_out" ]; then _pass "p10k 的 .zwc 不出現在 status"
    else _fail "p10k 的 .zwc 不出現在 status（rc=$_rc）" "$_out"; fi
    if _out=$(_zwc_cm apply --exclude=scripts "$_zwc_p10k"); then _rc=0; else _rc=$?; fi
    ...
    # exact 仍然有效：不是 .zwc 的雜物照刪，而且這次 apply 也要以 0 結束。
    printf 'x' > "$_zwc_p10k/internal/stray.txt"
    if _out=$(_zwc_cm apply --exclude=scripts "$_zwc_p10k"); then _rc=0; else _rc=$?; fi
    if [ "$_rc" -eq 0 ] && [ ! -e "$_zwc_p10k/internal/stray.txt" ]; then _pass "exact 仍會移除非 .zwc 的雜物，apply 退出碼 0"
    else _fail "exact 仍會移除非 .zwc 的雜物，apply 退出碼 0（rc=$_rc）" "$_out"; fi
fi
# 上面整段在網路 skip 時不會執行，這一條不靠網路：pattern 本身要在每個 POSIX 平台的 .chezmoiignore 裡。
for _os in $POSIX_OSES; do
