...
MUTANTS: list[Mutant] = [
    ...
    Mutant(
        ...
        path=".chezmoitemplates/platform.toml",
        old='{{-   $pkgManager = $archFamily | ternary "pacman" "apt" -}}',
        new='{{-   $pkgManager = $archFamily | ternary "apt" "pacman" -}}',
        layer="L1",
        rationale="ternary 的兩個分支寫反：Debian 走 pacman、Arch 走 apt，10-install-packages 在第一支腳本就中止",
    ),
    Mutant(
        name="neovim-mise-section-on-arch",
        path=".chezmoiscripts/run_before_50-neovim.sh.tmpl",
        old='{{ if ne $p.brewPrefix "" -}}\n#!/bin/zsh\n# Neovim (via mise)',
        new='{{ if $p.isPosix -}}\n#!/bin/zsh\n# Neovim (via mise)',
        layer="L2",
        rationale="mise 段的守衛退回 isPosix：Arch 家族上會 eval 一個不存在的 brew 並 mise use neovim（Must NOT #4/#8）",
    ),
    # ---- arch-family-support ----
    Mutant(
        name="platform-idlike-ignored",
        path=".chezmoitemplates/platform.toml",
        old='{{- $archFamily := or (eq $distro "arch") (has "arch" (splitList " " $distroLike)) -}}',
        new='{{- $archFamily := eq $distro "arch" -}}',
        layer="L1",
        rationale="只看 ID：正式版 omarchy（ID=omarchy）被當成 Debian，渲染 apt 與 Homebrew 腳本（M1，F2 的實測失敗）",
    ),
    Mutant(
        name="platform-idlike-substring",
        path=".chezmoitemplates/platform.toml",
        old='{{- $archFamily := or (eq $distro "arch") (has "arch" (splitList " " $distroLike)) -}}',
        new='{{- $archFamily := or (eq $distro "arch") (contains "arch" $distroLike) -}}',
        layer="L1",
        rationale="ID_LIKE 用子字串比對而不是逐字：任何 ID_LIKE 含 arch 字樣的發行版都走 pacman（M2）",
    ),
    Mutant(
        name="neovim-omarchy-check-gone",
        path=".chezmoiscripts/run_before_50-neovim.sh.tmpl",
        old="if pacman -Q omarchy-nvim &>/dev/null; then\n  exit 0\nfi\n",
        new="",
        layer="L2",
        rationale="少了 omarchy-nvim 的執行期判斷：omarchy 的 nvim 設定會被搬進 .bak 再 clone starter（M3，Must NOT #3）",
    ),
    Mutant(
        name="neovim-omarchy-check-inverted",
        path=".chezmoiscripts/run_before_50-neovim.sh.tmpl",
        old="if pacman -Q omarchy-nvim &>/dev/null; then",
        new="if ! pacman -Q omarchy-nvim &>/dev/null; then",
        layer="L7",
        rationale="判斷寫反：omarchy 上搬走設定、純 Arch 上什麼都不做（M3/M4）；只有 L7 的兩個 stub 分支看得出來",
    ),
    Mutant(
        name="pacman-syu",
    ...
    Mutant(
        ...
        layer="L2",
        rationale="Arch 家族的 neovim 由 pacman 來（50-neovim 沒有 mise 段）；清單漏掉它，純 Arch 上就沒有 nvim",
    ),
    Mutant(
