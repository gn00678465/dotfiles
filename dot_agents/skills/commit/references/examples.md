...
- [進階範例](#進階範例)
  ...
  - [含問題編號](#含問題編號)
  - [含共同作者](#含共同作者)
  - [複雜變更（大改動仍在上限內）](#複雜變更大改動仍在上限內)
- [不良範例（避免）](#不良範例避免)
  - [描述過於模糊](#-描述過於模糊)
  - [一個提交裝了兩個獨立目的](#-一個提交裝了兩個獨立目的)
  - [型別不同，但不該拆](#-型別不同但不該拆)
  - [描述加句號](#-描述加句號)
  - [未使用祈使句](#-未使用祈使句)
...
變更再大，正文一樣在上限內。此處用**預設的項目符號格式**；專案慣例另有正文格式時照專案的來。取捨的細節放 PR。

```
refactor(core): 重構核心引擎架構

- 採用依賴注入模式，業務邏輯抽離至 services 層
- 統一錯誤處理機制
- 初始化時間 +50ms，記憶體 -20%，API 回應時間無明顯變化
- 遷移：需更新 DI 容器設定，舊版 plugin 需重新編譯
```
...
登入功能與註冊表單修正**目的各自獨立，也能各自建置與還原**，所以拆：
...
- 新增 GET /items 的 page/size 參數
- 補上對應的整合測試
- 更新 API 文件的參數說明
```

`feat` + `test` + `docs` 三種型別，但測試與文件**單獨還原就失去意義**、單獨提交也
建置不出完整功能。相依性不完整就留在同一個提交——**型別不同不是拆分理由**。
