...
| 規模數字：行數、檔案數、`+N/-M`、「440 行測試」 | `git show --stat` 已經有，刪掉 |
...
單純重複的敘述**直接刪掉**，不需要另建文件。使用者在請求裡提到的數字也一樣：
它們描述變更的大小，不描述變更的內容，git 自己會算。
...
| 分支名稱建議 | 名稱 | 否 |
| 提交、commit、「幫我 commit」、「幫我寫 commit」 | 訊息並提交 | 是 |

「寫 commit」是提交；「寫 commit message」只是訊息。差別在 message 這個字。
...
交付文字。出現「重寫」「太長」「改短」字樣不等於授權改寫提交：**先把改寫後的訊息
交給使用者，問過再 amend**。對話中已有同範圍的批准就沿用，不重問。主分支保護
照樣適用：在 main／master 上不執行 amend。
...
2. **確認授權**：
   ...
   對話中已有**同範圍**批准就沿用，不重問。repo **沒有設定遠端**（`git remote` 為空）
   算未分享。有遠端但查不到（`git branch -r` 無對應分支、`git ls-remote` 不通）只代表
   **證據不足**，不能當作沒分享——證據不足時當已分享處理。
